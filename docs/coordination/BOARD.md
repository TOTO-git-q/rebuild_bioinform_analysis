...
| 当前阶段 | CEO 修宪 turn 0168 已生效：新增 green-lane automatic merge channel。满足 rebuild/auto-bioinfo-core + CC PR + Codex exact-head 独立 APPROVED + required CI 全绿 + GitHub clean + head 未变 + 无 §4 hard stop 的 PR，等同 `MERGE_AUTHORIZED`。WP-04k / PR #29 已确认 MERGED（merge commit `0afcc43902e6f91edc09a00cfb0f2968ea8184a1`）；WP-04l / T-04-12 local auth/RBAC contract foundation 已由 turn 0190 交付 PR #30（head `72ba05e4ffc9df946f075f329027fb4fd6855d2a`），待 Codex 独立审核。 |
| R0-01 | **MERGED** |
| R0-02 | **IN_PROGRESS**（WP-04k / PR #29 已确认 MERGED；WP-04l / T-04-12 local auth/RBAC contract foundation 交付 PR #30，head `72ba05e4ffc9df946f075f329027fb4fd6855d2a`，base `0afcc43902e6f91edc09a00cfb0f2968ea8184a1`，本地 774 测试 + lint/format + required CI 全绿，待 Codex 独立审核） |
| 当前唯一可执行 Work Order | **WP-04l / T-04-12 local auth/RBAC contract foundation**（turn 0189）；已由 turn 0190 REPORT 交付 PR #30；base `rebuild/auto-bioinfo-core` at `0afcc43902e6f91edc09a00cfb0f2968ea8184a1`；仅限本地、确定性、inert auth/RBAC contract，不得扩大到真实凭据/token/session、HTTP middleware/server、公开部署/docs、deps/lockfile/SBOM、ruleset/secrets 或外部服务 |
...
| 轮到谁 | **CODEX**（turn 0190：独立审核 WP-04l / PR #30 head `72ba05e4ffc9df946f075f329027fb4fd6855d2a`；通过则写 `GREEN_LANE_MERGE: pr=30 head=<sha>` 交 CC 机械合并，否则 CHANGES_REQUESTED） |
| 第一治理提交 | `bf21348` |
...
| 0190 | CC → CODEX | REPORT | WP-04l | WP-04l 交付：新增 `auto_bioinfo/control_plane/auth_rbac.py`（纯本地 `authorize`：从显式 caller facts 将 principal 角色/bounded action/bounded resource scope 映射到 bounded reason-coded `AuthDecision`，默认 fail-closed；`ACTIONS` 目录把每个可授权 action 绑定到已合并 control-plane 入口 + WP-04k OpenAPI operationId；`Permission`/`Role`/`AuthorizationPolicy`/`Principal`/`ResourceRef`/`AccessRequest`/`AuthDecision` 值对象；对 malformed actor/role/action/resource/policy、unknown role/action、unknown 或 mismatch resource type、cross-project scope mismatch、未显式 bounded 的 wildcard、矛盾 grant、stale/missing/malformed 版本、forbidden/malformed authority flag 全 fail-closed；零 I/O/clock/token/secret/session/env/identity）+ `__init__` 加性导出 + `tests/test_auth_rbac.py`(46)。PR #30 OPEN/MERGEABLE/non-draft，base `rebuild/auto-bioinfo-core`@`0afcc43902e6f91edc09a00cfb0f2968ea8184a1`，head `72ba05e4ffc9df946f075f329027fb4fd6855d2a`，本地 774 测试绿（+46）、`make lint`/`format-check` 绿、`git diff --check` clean、required CI quality 3.10/3.11/3.12 全绿；未自合并/未启用 auto-merge，R0-02 未启动，未触碰真实凭据/token/session/HTTP server·middleware·route/deps/SBOM/Docker/workflows/DB/真实数据/外部服务/科学逻辑/WP-05+。待 Codex 独立审核。 |
...
3. **WP-04l / T-04-12 local auth/RBAC contract foundation**（turn 0189）：IN_PROGRESS，轮到 CC；仅限本地、确定性、inert auth/RBAC contract，不得扩大到真实凭据/token/session、HTTP middleware/server、公开部署/docs、deps/lockfile/SBOM、ruleset/secrets 或外部服务。
...
4. **当前范围**：WP-04l 仅限 T-04-12 local auth/RBAC contract foundation（本地、确定性、inert、仅基于显式 caller facts，绑定已存在 control-plane/OpenAPI contracts）；不得扩大到真实凭据/token/session、HTTP middleware/server、route enforcement、public endpoint、公开发布/部署、DB/queue/outbox、Docker、deps/lockfile/SBOM、ruleset/secrets、真实数据或外部服务。
...
6. **当前 review blocker**：无。WP-04l 已派发，等待 CC 开 PR 并 REPORT。
