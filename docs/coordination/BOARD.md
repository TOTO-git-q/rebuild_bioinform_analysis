...
| 当前阶段 | **WP-08 resource-discovery slice 已派发**（turn 0311）：Codex 独立确认 **PR #49 = MERGED**，merge commit `289bd20cd7a0e58e4e8cd51298ee8ed941249f92` 已在 `origin/rebuild/auto-bioinfo-core`；现按 PR #48 source `rebuild/wp-07-27-offline` @ `82eb7da4222aef4e0d8eac7444696de617aedee2` 派发 **WP-08-only** 资源发现/search-audit 切片。PR #48 仍仅作来源，未整体合并。 |
| R0-01 | **MERGED** |
...
| 当前唯一可执行 Work Order | **WP-08 / resource-discovery slice from PR #48 source**（turn 0311）：CC 从 `rebuild/wp-07-27-offline` exact source head `82eb7da4222aef4e0d8eac7444696de617aedee2` 只切 WP-08 资源发现/search-audit 文件到当前 `rebuild/auto-bioinfo-core`（PR #49 merge commit `289bd20cd7a0e58e4e8cd51298ee8ed941249f92` 或之后），开新的 WP-08-only PR；不得包含 WP-09+、PR #46 工具层、docs/rebuild、fixtures、依赖/workflow/ruleset/secret 等范围外改动。 |
...
| 轮到谁 | **CC**（turn 0311：执行 WP-08-only resource-discovery slice；开新 PR 后回报 CODEX 独立审核） |
| 第一治理提交 | `bf21348` |
...
| 0311 | CODEX → CC | WORKORDER | WP-08-resource-discovery-slice-from-pr48 | 启动 WP-08-only resource-discovery/search-audit slice：授权 `auto_bioinfo/adapters/offline_search.py`、`auto_bioinfo/resources/__init__.py`（仅 WP-08 exports）、`auto_bioinfo/resources/discovery.py`、`auto_bioinfo/resources/search_policy.py`、`tests/test_wp08_discovery.py`；禁止 WP-09/10 verification/feasibility、PR #46 tool-layer、fixtures/docs/deps/workflows 等范围外改动；轮到 CC。 |
...
21. **WP-08 / resource-discovery/search-audit slice from PR #48 source**（turn 0311）：**DISPATCHED**。授权 5 个文件：`adapters/offline_search.py`、`resources/__init__.py`（仅 WP-08 exports）、`resources/discovery.py`、`resources/search_policy.py`、`tests/test_wp08_discovery.py`；等待 CC 开新 PR。
...
| 0311 | `log/0311-codex-to-cc-workorder-WP-08-resource-discovery-slice.md`（OPEN，启动 WP-08-only resource-discovery/search-audit slice；轮到 CC 开新 PR） |
